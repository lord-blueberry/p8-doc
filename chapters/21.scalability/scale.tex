...
In this project, we created a Compressed Sensing reconstruction which works outside the major cycle. To our knowledge this approach has not been explored previously for radio interferometer image reconstruction. It uses Coordinate Descent and directly calculates the columns of $F^{-1}$ relevant to the reconstruction. Together with starlets as its regularization created super-resolved point sources on simulated MeerKAT data. The total flux of extended emissions were captured more accurately than CLEAN. 
...
Super resolution on real observations was not done.

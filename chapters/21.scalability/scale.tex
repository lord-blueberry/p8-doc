...
Exploited the starlet for a heuristic scheme, which we can use to find likely relevant columns of the Fourier transform matrix. Our algorithm is not fixed to starlet regularization. The same starlet heuristic could be used with different regularizations, for example a Gaussian mixture model.
...
Major Cycle Compressed Sensing algorithms have also shown super-resolution performance on real-world observations, but also at a higher runtime cost compared to CLEAN. The main goal of this project was to find a new architecture which reduces the runtime cost of Compressed Sensing reconstructions. In the context of MeerKAT, this was not possible. Ironically, for MeerKAT reconstructions the runtime costs of Coordinate Descent can be improved by using the Major Cycle architecture and reduce its memory requirement.

That does not mean the new architecture cannot beat CLEAN or the Major Cycle. It scales independently of the image size. The cost difference of the two Architectures boils down to the ratio between Visibilities and pixels. For MeerKAT reconstructions, the ratio is skewed towards Visibilities. If we increase the number of pixels in the reconstruction, we increase the cost-effectiveness of our direct Fourier transform architecture.

Increasing the number of pixels is only useful to a limit. Decreasing the number of Visibilities however, might yield better results: The large number of Visibilities are virtually guaranteed to have redundant information. A portion of the total runtime is spent for Visibilities, which do not contain any new information for the image. An algorithm may be able to reconstruct an approximate image from a fraction of the total Visibilities. In this environment, our new architecture may become viable for reducing runtime costs.

Our new architecture with Coordinate Descent has the trouble of being too precise. We do not need the Fourier transform calculated up to the limit of floating point accuracy. The instrument will have more noise than that. The Major Cycle has the right idea with using an approximate Fourier transform. The key for our approach is bringing back approximations and try to keep the costs down.

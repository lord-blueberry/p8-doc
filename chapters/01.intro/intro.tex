...
In a Major Cycle, the Visibilities get transformed into image space, CLEAN deconvolves the image with the $PSF$, and the residual image gets transformed back into Visibilities. It minimizes roughly the objective \eqref{intro:clean}\footnote{ In reality, CLEAN uses a number of heuristics which are hard to formalize. This objective is a mere approximation to highlight its properties in our context.}. As we will see, continually cycling between Visibility an image space has some advantages, which Compressed Sensing algorithms inherit by also using the non-uniform FFT. We show the advantages of the non-uniform FFT at the example of CLEAN and the Major Cycle architecture, and discuss the implications for Compressed Sensing algorithms.
...
CLEAN can work with a constant PSF. For Radio Interferometers, the PSF changes over the image. But we only need to approximate the deconvolution in the Major Cycle architecture.

Inherits that the non-uniform FFT gets more accurate with further cycles. 

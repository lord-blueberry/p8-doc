...
Image reconstruction problems arise in a variety of fields. Imaging instruments measure noisy, corrupted data which a reconstruction algorithm should remove. In Radio Astronomy, interferometers pose a reconstruction problem which is under active research. Interferometers measures a noisy, incomplete set of Fourier Components. A reconstruction algorithm has multiple possible images that fit the measurements. It has to decide the most likely image that fits the measurements.

The new MeerKAT radio interferometer has all these problems, but poses the reconstruction problem on a data volume. Hundreds of gigabytes of Measuerements get reconstructed to a single image. The CLEAN algorithms are the current state-of-the-art reconstruction algorithm for MeerKAT. A heuristic and gives its "best guess". How far the reconstructed image is from the observed one is hard to know.


The theory of Compressed Sensing gives us the ground work to actually retrieve the observed image. We can model the image reconstruction as a convex optimization problem. It gives us guarantees that under the right condition, we can not only reconstruct the observed image, but may retrieve it above the accuracy limit of the instrument. In recent years, Compressed Sensing based algorithms have been developed. They showed super-resolution quality on real world interferometer observations. But they come at a higher runtime cost. Research into speeding Compressed Sensing reconstructions up for large scale reconstructions, and remove CLEAN.

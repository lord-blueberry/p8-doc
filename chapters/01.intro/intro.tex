...
%This has led research to look for a scalable and distributable reconstruction algorithm. 
The current state-of-the art approach for MeerKAT is based on 
CLEAN\cite{rich2008multi, rau2011multi}. It is not the best in reconstruction quality, but scales to MeerKAT problem sizes.

The theory of Compressed Sensing\cite{candes2006robust, donoho2006compressed} has led rise to a new class of algorithms. They demonstrated super-resolution capability on real-world observations\cite{girard2015sparse, dabbech2018cygnus}. Compared to CLEAN, Compressed Sensing algorithms have the potential to increase the accuracy of the reconstruction from the same measurements. 
But they come at a higher runtime cost. 
How to create a scalable Compressed Sensing reconstruction is still under active research.

CLEAN and state-of-the-art Compressed Sensing algorithms use the non-uniform FFT approximation\cite{kunisnonequispaced, pratley2017robust} to cycle between measurements and image space. Compressed Sensing algorithms need more cycles to converge to an image, which is one source of the higher runtime costs. In this project we investigate three alternatives to the non-uniform FFT for Radio Interferometers. We created a proof-of-concept algorithm which does not need the non-uniform FFT during optimization. Instead, we use the direct Fourier Transform which potentially leads to a distributable Compressed Sensing algorithm. Sadly, our new algorithm cannot reduce the runtime costs of large scale reconstructions.
...
No single step in solving the problem.

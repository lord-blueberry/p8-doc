...
State-of-the art Compressed Sensing algorithms also use the non-uniform FFT and inherit similar advantages from the Major Cycle. The error of the Fourier approximation gets continually reduced over several cycles. Although Compressed Sensing algorithms do not use a deconvolution, and do not benefit in that regard.



As we will see in the next section \ref{meerkat}, Compressed Sensing can also reduce the error of the non-uniform FFT approximation by cycling between Visibilities and image. Although state-of-the-art algorithms do use a deconvolution

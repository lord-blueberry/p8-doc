...
Image reconstruction problems arise in a variety of fields. Instruments measure noisy, corrupted data and a reconstruction algorithm is tasked with removing any corruption. In Radio Astronomy, interferometers pose a reconstruction problem which is under active research. They do not measure pixels of the image directly. Instead, they measures a noisy, incomplete set of Fourier Components. Several different images fit the measurements. A reconstruction algorithm has to find the observed image from a possibly large set of candidates.

The new MeerKAT instrument poses the reconstruction problem on a new data volume. Fourier Components of several gigabytes worth of data should get reconstructed to a single image.
Scalable reconstruction algorithm.


The CLEAN algorithms\cite{rich2008multi, rau2011multi} are the current state-of-the-art reconstruction algorithm for MeerKAT. A heuristic and gives its "best guess". Although CLEAN has is only distributable to a limit, it is currently one of the cost-effectvie approaches.

The theory of Compressed Sensing\cite{candes2006robust, donoho2006compressed} gives us the ground work to actually retrieve the observed image. We can model the image reconstruction as a convex optimization problem. It gives us guarantees that under the right condition, we can not only reconstruct the observed image, but may retrieve it above the accuracy limit of the instrument. In recent years, Compressed Sensing based algorithms have been developed. They showed super-resolution quality on real world interferometer observations\cite{girard2015sparse, dabbech2018cygnus}. But they come at a higher runtime cost.


CLEAN and state-of-the-art Compressed Sensing algorithms use the non-uniform FFT approximation\cite{kunisnonequispaced, pratley2017robust} to cycle between measurements and image space. Compressed Sensing algorithms need more cycles to converge to an image, which is one source of the higher runtime costs. There is little research investigating alternatives to the non-uniform FFT. In this project we investigate three alternatives to the non-uniform FFT for Radio Interferometers. We created a proof-of-concept algorithm which does not need the non-uniform FFT during optimization. Instead, we use the direct Fourier Transform which potentially leads to a distributable Compressed Sensing algorithm. Sadly, our new algorithm cannot reduce the runtime costs of large scale reconstructions.

The non-uniform FFT leads to lower runtime costs. The cyclic nature of the non-uniform FFT currently does not lend itself to distribution on a large scale.. Current research focuses on getting the non-uniform FFT to distribute. Creating both a cost-effective and distributable reconstruction algorithm is still an open problem. 
Our algorithm has the opposite problem. It can be improved further. 
We show the promises and limitations of alternatives to the non-uniform FFT. 
Alternatives have potentials worth investigating.

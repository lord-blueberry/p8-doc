...
The main drawback of the simple approach is the explicit calculation of the whole Fourier Transform Matrix $F^{-1}$.

In practical applications it works well enough and is primed for a lot of heuristics. It is robust and primed for heuristics.

The  the algorithm has been working well enough and is used for VIS-CS \cite{felix2017compressed} compressed sensing reconstruction. \ref 
The simple version of coordinate descent simply runs through every column in sequence. The simple version of the algorithm simply calculates the column of the Fourier Transform Matrix  for each pixel.

...
The starlet transform is based on the starlet wavelet shown in figure \ref{cd:starlets:figure}. It decomposes an image into multiple layers, each representing parts of the image at different scales. The lower layers contain small structures like point sources, while the higher layers contain extended emissions like hydrogen clouds. The transformation from starlet space back into image space is a simple addition: All layers added together result in the original image. The transformation from image to starlet space is done with a multi-scale wavelet decomposition, which is explained in more detail.
...
The different layers are calculated with a multi-scale wavelet decomposition and 

 It separates small structures like point sources from extended emissions like hydrogen clouds into separate images.  which uses the starlet wavelet shown in figure \ref{cd:starlets:figure}. The multi-scale decomposition keeps 

...
In this section, we put together the starlet transform and the Coordinate Descent algorithm to our solution. 
...
Our algorithm is split in two separate Coordinate Descent loops. The first loop iterates over the starlet components of a single layer. It is where we use the starlet transform as a heuristic for likely non-zero components. The second Coordinate Descent loop iterates over all non-zero components. In this step with the non-negative starlet, a lot of bases drop to zero and we arrive at a more sparse solution.

\subsubsection{Iteration Scheme}

We discuss the objective function and how we only need the columns of $F$ with a non-zero starlet component. Then, we show the heuristic , we discuss how The objective function \eqref{cd:starlet} reconstructs the image in the starlet space $\alpha$. The $D$ matrix represents all our starlet basis functions and the image can be reconstructed with $x =D\alpha$. 

\begin{equation}\label{cd:starlet}
\underset{\alpha}{minimize} \: \left \| V - FD\alpha \right \|_2^2 + \lambda \left \| \alpha \right \|_1 \\
\end{equation}

 Currently, the product $FD\alpha$ requires Fourier columns for every non-zero pixel. This can be improved. 

...
Let us look at the lowest starlet layer first, how it is calculated from the image, and how we use it for image reconstruction. The starlet layer $w_0$ gets calculated in two steps, shown in \eqref{cd:starlet:w0}. First, we take the scaling function, shown in image \ref{cd:starlets:scaling} and denoted ad $B_0$, and convolve the image with it, resulting in the convolved image of layer 0 $c_0$. The final step is subtracting $c_0$ from $x$. This removes larger structures from the image and the starlet layer $w_0$ is left with only the smaller structures. 

$w_0$  has the same dimensions as the input image. It contains the point sources of the image convolved with the starlet wavelet \ref{cd:starlets:wavelet}. The last step for finding the point sources $\alpha_0$, is to deconvolve $w_0$.

This is difficult. However we can approximate it with a shrink operation. FISTA scheme of \cite{girard2015sparse}.
We use the soft thresholding operation as a probability distribution. We take the $w_0$ image and see likely relevant pixels and use Coordinate Descent to find its optimum value. A visual example \ref{cd:heuristic:figure}.
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Dirty map and $w_0$ starlet layer after thresholding. $w_0$ can be interpreted as a probability distribution for point source locations.}
	\label{cd:heuristic:figure}
\end{figure}

The individual pixels are not independent. The image \ref{cd:heuristic:starlet} has a high probability for an area of pixels, although there are only two point sources in the image.

Starlet coefficient $\alpha_0$ convolved with the starlet wavelet gets back to $w_0$
...
The decomposition in total is chaining the blurring of the scaling function $B_i$ at different scales. Results in the decomposition shown in equation \eqref{cd:starlet:decomposition}
...
The synthesis transform however is easy. We simply add the starlet layers back together \eqref{cd:starlet:backwards}.
...
In this work, we did not simply use the starlet wavelet for reconstruction, we modified it to be non-negative. Our images have to be non-zero.

\subsubsection{Handling the non-negativity constraint}
...
This work orientates itself after the 

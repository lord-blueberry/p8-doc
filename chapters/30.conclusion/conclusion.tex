...
The theory of Compressed Sensing is primed for exploiting redundant information. To our knowledge, the Radio Astronomy community does not investigate how Compressed Sensing algorithms could leverage the inherent redundancy in MeerKAT observations. State-of-the-art Compressed Sensing algorithms use the $w$-stacking algorithm and include all redundant information in the reconstruction. 

There are many potential ways of improving Compressed Sensing reconstruction by replacing the non-uniform FFT. But as we have seen in this project, any alternative to the non-uniform FFT likely needs more approximations on top. In the case of the direct Fourier Transform, it needs further approximations on top of accounting for redundancy before it may even be tractable for MeerKAT reconstructions. 

Proof of concept algorithm before it is mature enought to beat algorithms based on the non-uniform FFT.
